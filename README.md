...
Androidは既存のリリース署名設定（`android/app/build.gradle.kts` が読む
`key.properties`）をそのまま使い、GitHub Actions
（`.github/workflows/playstore.yml`、`ubuntu-latest`ランナー）と fastlane
（`android/fastlane/`）でGoogle Playの内部テストトラックにアップロードする。
以下のRepository Secretsを登録した上で Actions タブから
`Deploy to Play Store (internal testing)` ワークフローを手動実行する。

| Secret | 内容 |
| --- | --- |
| `ANDROID_KEYSTORE_BASE64` | リリース用keystore(`.jks`)ファイルをbase64エンコードした文字列（`base64 -i release.keystore \| pbcopy`） |
| `ANDROID_KEYSTORE_PASSWORD` | keystoreのストアパスワード |
| `ANDROID_KEY_ALIAS` | 署名キーのエイリアス |
| `ANDROID_KEY_PASSWORD` | 署名キーのパスワード |
| `PLAY_STORE_JSON_KEY_BASE64` | Google Play Console のサービスアカウント用JSONキーをbase64エンコードした文字列 |
| `ANDROID_PACKAGE_NAME` | パッケージ名（任意、既定値は `com.fush1m1.forge2dgame`） |

サービスアカウントJSONキーは Play Console の「ユーザーとアクセス権限」→
「サービスアカウントを招待」から発行し、対象アプリに対して
「製品版・非公開テスト版・限定公開版へのリリースの管理」権限を付与しておくこと。
**Google PlayのAPI経由アップロードは既存アプリへの追加リリースにしか使えない
仕様のため、初回だけは手動でPlay Consoleにアプリを作成しAAB/APKを
アップロードしておく必要がある。**
